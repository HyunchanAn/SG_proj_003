...
def run_test(sample_per_folder=5):
    ...
    folders = ["260420_BA_reflect", "260420_#4_reflect", "260420_HL_reflect", "260420_SM_reflect", "260420_2B_reflect"]
    ...
    print(f"{'Folder':<20} | {'Sample':<5} | {'Pred':<5} | {'Ra':<6} | {'Gloss':<6} | {'Match'}")
    print("-" * 70)
    ...
    for folder_name in folders:
        ...
        for img_path in images:
            try:
                ...
                is_correct = gt_label in pred_label # 예측된 제품명에 정답 라벨이 포함되어 있는지 확인
                if is_correct:
                    correct += 1
                total += 1
                
                print(f"{gt_label:<20} | {img_path.name[-8:]} | {pred_label:<5} | {analysis['roughness']:.3f} | {analysis['gloss']:.1f} | {is_correct}")
