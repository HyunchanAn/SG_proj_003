...
# --- 설정 및 스타일 ---
st.set_page_config(
    page_title="V-SAMS Surface Analysis Lab",
    page_icon="🔍",
    layout="wide"
)
...
with st.sidebar:
    ...
    masking_mode = st.radio("마스킹 모드", ["자동 (Auto)", "수동 박스 (Manual Box)"], horizontal=True)
    ...
    if input_mode == "테스트 데이터셋":
        ...
        if test_root.exists():
            ...
            selected_img_path = st.selectbox("이미지 선택", [img.name for img in images])
...
if target_image:
    ...
    with col1:
        ...
        if masking_mode == "자동 (Auto)":
            ...
            st.markdown("**수동 가이드:** 1. **실제 동전** 영역 드래그 → 2. **반사광** 영역 드래그")
    ...
    with col2:
        if st.session_state.do_analysis:
            if masking_mode == "수동 박스 (Manual Box)" and (boxes is None or len(boxes) < 2):
                ...
                st.image(overlay_img, use_container_width=True, caption="Green: Real Coin | Blue: Reflection")
                ...
                if match:
                    ...
                    st.info(f"물성 정보: Ra {match['roughness_avg']:.4f} | Gloss {match['gloss_avg']:.2f}")
                else:
    ...
    if st.session_state.do_analysis:
        new_entry = {
            "파일명": selected_img_path if input_mode == "테스트 데이터셋" else "Uploaded",
