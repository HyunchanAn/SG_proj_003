...
if evaluator.device.type == "cpu":
    st.warning(
        "⚠️ GPU 메모리 부족으로 인해 CPU 모드로 작동 중입니다. 분석 속도가 느려질 수 있습니다."
    )
else:
...
with st.sidebar:
    ...
    source_dir = st.text_input(
        "원본 이미지 폴더 경로", value=str(project_root / "test_260420_surface")
    )
    target_dir = st.text_input(
        "저장될 데이터셋 경로", value=str(project_root / "dataset" / "verified")
    )
    ...
    if st.session_state.files_to_process:
        progress = (st.session_state.file_index + 1) / len(
            st.session_state.files_to_process
        )
        st.progress(progress)
        st.write(
            f"진행도: {st.session_state.file_index + 1} / {len(st.session_state.files_to_process)}"
        )
...
if st.session_state.files_to_process:
    ...
    with col1:
        ...
        if mode == "수동 박스 (Manual Box)":
            ...
            if (
                st.session_state.current_analysis is None
                or st.session_state.current_analysis.get("path") != current_file_path
            ):
        ...
        if st.session_state.current_analysis:
            ...
            overlay_img = evaluator.get_overlay_image(
                img, st.session_state.current_analysis
            )
            st.image(
                overlay_img,
                width="stretch",
                caption="Blue: Real Coin | Red: Reflection",
            )
    ...
    with col2:
        ...
        if st.session_state.current_analysis:
            ...
            with st.container():
                ...
                st.write(
                    f"반사광 감지: `{'성공' if analysis['has_reflection'] else '실패'}`"
                )

                st.divider()
                st.markdown(
                    f"**🤖 AI 예측 결과: <span style='color:#00ff00; font-size:1.2em;'>{analysis.get('predicted_label', 'Unknown')}</span>**",
                    unsafe_allow_html=True,
                )
            ...
            label = st.selectbox(
                "표면 종류 선택", surface_types, index=default_label_idx
            )
            mask_quality = st.radio(
                "마스킹 품질", ["Good ✅", "Bad ❌", "Uncertain ❓"], horizontal=True
            )
            ...
            if c1.button("확정 및 저장 💾", type="primary"):
                ...
                if (
                    ...
                    < len(st.session_state.files_to_process) - 1
                ):
