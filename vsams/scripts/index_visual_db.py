...
def index_dataset(dataset_root='dataset/260414', output_path='vsams/data/visual_library.pth'):
    print(f"Initializing SAM and Classifier...")
    # 1. Setup Models
    ...
    product_folders = sorted([f for f in os.listdir(dataset_root) if os.path.isdir(os.path.join(dataset_root, f))])
    print(f"Found {len(product_folders)} product folders.")
    
    for folder in tqdm(product_folders, desc="Processing Products"):
        folder_path = os.path.join(dataset_root, folder)
        product_name = folder.split('_', 2)[-1] if len(folder.split('_')) >= 3 else folder
        ...
        id_images = [f for f in os.listdir(folder_path) if f.lower().endswith('.jpg') and f not in surface_images]
