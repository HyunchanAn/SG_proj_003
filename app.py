...
# --- Config ---
st.set_page_config(page_title="V-SAMS Core Hub", page_icon="🛡️", layout="wide")
...
if uploaded_file:
    ...
    with col1:
        ...
        st.info(
            "파란색 박스로 실제 동전 을, 빨간색 박스로 반사된 이미지 를 드래그하여 지정하세요."
        )
    ...
    with col2:
        ...
        if result and "error" not in result:
            ...
            if closest:
                st.info(
                    f"📚 유사 제품 (DB): {closest['product_name']} (오차: {closest['distance']:.4f})"
                )

            # 감지 영역 시각화 (동전/반사광 위치 확인용)
            st.image(
                evaluator.get_overlay_image(img.copy(), result),
                caption="감지된 분석 영역 (Blue: Coin, Red: Reflection)",
            )
            ...
            st.info(
                "사이드바의 캔버스에서 직접 동전과 반사광 영역을 사각형으로 그려주세요."
            )
